...
namespace LicenseManagement.API.Services
{
    public class LicenseReminderService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            // انتظر دقيقة واحدة عند البدء ثم شغّل
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogError(ex, "❌ Error sending license reminder emails");
                }
        ...
        private async Task SendReminderEmailsAsync()
        {
            ...
            // تقسيم حسب الخطورة
            var critical = expiring.Where(l => (l.ExpiryDate!.Value.Date - today).Days <= 30).ToList();
            var warning  = expiring.Where(l => (l.ExpiryDate!.Value.Date - today).Days is > 30 and <= 60).ToList();
            var notice   = expiring.Where(l => (l.ExpiryDate!.Value.Date - today).Days is > 60 and <= 90).ToList();
            ...
            _logger.LogInformation($"📧 Reminder email sent: {expiring.Count} licenses ({critical.Count} critical)");
        }
            ...
            DateTime today)
        {
            static string Row(LicenseManagement.API.Models.License l, DateTime today)
            {
                ...
                var bgRow = days <= 30 ? "#FEF2F2" : "#FFFBEB";
                return $@"
                <tr style='background:{bgRow}'>
                    ...
                    <td style='padding:10px 14px;font-size:12px;color:#6B7280'>{l.ExpiryDate!.Value:yyyy-MM-dd}</td>
                    <td style='padding:10px 14px;font-size:14px;font-weight:700;color:{color}'>{days} يوم</td>
                </tr>";
            }

            static string Section(string title, string color, List<LicenseManagement.API.Models.License> items, DateTime today)
            {
                if (!items.Any()) return "";
...
<div style='max-width:680px;margin:32px auto;background:#FAF6F0;border:1px solid #D6CCBE'>
    ...
    <!-- Header -->
    <div style='background:#1A3A2A;padding:24px 28px;display:flex;align-items:center;gap:12px'>
        <div>
    ...
    <div style='background:#254D38;padding:14px 28px;display:flex;gap:24px'>
        <div style='text-align:center'>
            ...
            <div style='font-size:10px;color:#8BAF97;text-transform:uppercase;letter-spacing:.8px'>حرجة (30 يوم)</div>
        </div>
        <div style='width:1px;background:#2F6045'></div>
        <div style='text-align:center'>
            <div style='font-size:24px;font-weight:700;color:#D97706'>{warning.Count}</div>
            <div style='font-size:10px;color:#8BAF97;text-transform:uppercase;letter-spacing:.8px'>تحذير (60 يوم)</div>
        </div>
        <div style='width:1px;background:#2F6045'></div>
        <div style='text-align:center'>
            <div style='font-size:24px;font-weight:700;color:#059669'>{notice.Count}</div>
            <div style='font-size:10px;color:#8BAF97;text-transform:uppercase;letter-spacing:.8px'>تنبيه (90 يوم)</div>
        </div>
    ...
    <div style='background:#1A3A2A;padding:14px 28px;text-align:center'>
        ...
        <a href='https://gallant-celebration-production-e728.up.railway.app'
           style='display:inline-block;margin-top:8px;padding:7px 18px;background:#BA7517;
                  color:white;font-size:11px;font-weight:700;text-decoration:none;letter-spacing:.5px'>
            فتح النظام ←
        </a>
    </div>
